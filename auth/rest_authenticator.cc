...
// To ensure correct initialization order, we unfortunately need to use a string literal.
    ...
    // TODO is it really a good idea it just remove 2/3 lines to not have it in the constructor and it doesn't look that
    //  good to have it a method that must be called before start and other method to avoid failure
    void rest_authenticator::set_authenticator_config(authenticator_config &ac) {
        _authenticator_config = authenticator_config(ac);
    }
        ...
        f) {
            try {
                ...
                if (!salted_hash || !passwords::check(password, *salted_hash)) {
                    ...
                    // TODO add usage of  std::auto_ptr /  std::shared_ptr to automanage deletion of objects out of scope
                    bool user_to_create = res->empty();

                    // TODO manage retry?
                    // TODO timeout
                    plogger.info("Authenticating username {} from rest endpoint", username);
                    return _rest_http_client.connect().then(
                            [this, user_to_create, username, password](rest_http_client::connection *c) {
                                return c->do_get_groups(username, password).then(
                                        [this, user_to_create, username, password](std::vector <std::string> groups) {
